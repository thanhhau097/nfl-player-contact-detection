...
class Config:
    ...
    NAME = "NFLC-" + "simple-xgb-baseline-2"
...
for nnmodel in ["tf_efficientnetv2_s.in21k_ft_in1k", "tf_efficientnetv2_b2", "resnet50"]:
...
def func(x_list):
    score = matthews_corrcoef(df['contact'], df['preds_score'] > x_list[0])
    return -score
...
oof_pred = fit_xgboost(cfg, df, feature_cols, cfg.xgb_params, add_suffix="")
# df.to_csv("xgboost_result.csv", index=False)
...
df['preds_score'] = oof_pred.copy()
df = df.sort_values('pairs').reset_index(drop=True)

# Ground contact sampling
df_new = df.copy()
df_new = df_new[['contact_id', 'preds_score']]
df_new = pd.concat([df_new[df_new.contact_id.str.contains('G')][::3], df_new[~df_new.contact_id.str.contains('G')]])
del df['preds_score']
df = df.merge(df_new, on='contact_id', how='left')
df['preds_score'] = df['preds_score'].astype('float')
df = df.groupby('pairs_nostep').apply(lambda group: group.interpolate(limit_direction='both'))
...
for i in df.query("distance>1.85").index:
    df.loc[i, 'preds_score'] = 0

x0 = [0.5]
result = minimize(func, x0,  method="nelder-mead")
score = round(matthews_corrcoef(df['contact'], df['preds_score'] > result.x[0]), 5)
...
score = round(matthews_corrcoef(df['contact'], preds), 5)
print(f"Window {window} Accum {accum} - score: {score}")
df['preds_score'] = preds
df.to_csv("current_pred.csv", index=False)
