...
class DataArguments:
    ...
    num_frames: int = field(
        default=13, metadata={"help": "num frames used to train CNN, should be odd number"}
    )
    frame_steps: int = field(
        default=4, metadata={"help": "number of skipped frames between two selected frames"}
    )
    num_center_frames: int = field(
        default=3,
        metadata={"help": "num center frames used to train CNN, not including middle frame"},
    )
    ...
    num_cache: int = field(default=-1, metadata={"help": "num train frames cached on RAM"})
