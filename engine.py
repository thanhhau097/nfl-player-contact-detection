...
class CustomTrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            observed_batch_size = find_batch_size(logits)
        ...
        if all_preds is not None:
            all_preds = nested_truncate(all_preds, observed_num_examples)
        if all_labels is not None:
            all_labels = nested_truncate(all_labels, observed_num_examples)
...
def postprocess_pred_output(predictions: np.ndarray, labels: np.ndarray):
...
def compute_metrics(eval_preds):
    ...
    predictions, labels = postprocess_pred_output(predictions, labels)
    auc = roc_auc_score(labels, predictions)
    score = matthews_corrcoef(labels, predictions > 0.5)
    return {"AUC": auc, "matthews_corrcoef": score}
