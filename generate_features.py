...
def main(csv_folder: str):
    ...
    for cluster in CLUSTERS:
        ...
        for helmet_view in ["Sideline", "Endzone"]:
            ...
            tmp_helmets = tmp_helmets.groupby("game_play").apply(
                lambda x: add_step_pct(x, cluster)
            )
            tmp_helmets = tmp_helmets[tmp_helmets["view"] == helmet_view]

            tmp_helmets["helmet_id"] = (
                tmp_helmets["game_play"]
                + "_"
                + tmp_helmets["nfl_player_id"].astype(str)
                + "_"
                + tmp_helmets["step_pct"].astype(str)
            )

            tmp_helmets = (
                tmp_helmets[["helmet_id", "left", "width", "top", "height"]]
                .groupby("helmet_id")
                .mean()
                .reset_index()
            )
            for player_ind in [1, 2]:
                df["helmet_id"] = (
                    df["game_play"]
                    + "_"
                    + df["nfl_player_id_" + str(player_ind)].astype(str)
                    + "_"
                    + df["step_pct"].astype(str)
                )

                df = df.merge(tmp_helmets, how="left")
                df.rename(
                    columns={
                        i: i + "_" + helmet_view + "_" + str(cluster) + "_" + str(player_ind)
                ...
                feature_cols += [
                    i + "_" + helmet_view + "_" + str(cluster) + "_" + str(player_ind)
                    for i in ["left", "width", "top", "height"]
                ]
            del tmp_helmets
